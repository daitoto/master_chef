...
def extractSlots(text, state):
	...
	pattern = re.compile('(水煮肉片|水煮鱼|麻婆豆腐|鱼香肉丝|蚂蚁上树|辣子鸡|回锅肉|酸汤肥牛|糯米蒸排骨|肉末茄子|粉蒸排骨|干煸肥肠|青椒肉丝|鱼香茄子|'
		 				+'剁椒鱼头|小炒肉|湘西土匪鸭|干锅花菜|啤酒鸭|麻辣小龙虾|菠萝咕咾肉|蜜汁叉烧肉|腊肠煲仔饭|盐焗鸡|梅菜扣肉|糖醋排骨|宫(保|爆)鸡丁|番茄炒鸡蛋|红烧肉|糯米丸子|'
		 				...
		 				+'过桥米线|汽锅鸡|菠萝饭|黑三剁|京酱肉丝|宫(保|爆)虾球|炸酱面|褡裢火烧|葱爆羊肉|拔丝鸡盒|醋溜木须|八宝饭|醋溜白菜|炸八块|红焖羊肉|胡辣汤|'
		 				+'带把肘子|油泼裤带面|烩肉三鲜|煨鱿鱼丝|清蒸武昌鱼|排骨藕汤|皮条鳝鱼|糍粑鱼|酸辣藕带|热干面|南昌炒米粉|粉蒸肉|藜蒿炒腊肉|莲花血鸭|'
		 				+'西红柿炒鸡蛋|番茄炒蛋|西红柿炒蛋|水煮牛肉|西红柿牛腩|番茄牛腩|红烧牛肉|萝卜炖牛肉|土豆炖牛肉|干煸牛肉丝|红烩牛肉|咖喱牛肉)$')
	if re.search(pattern, text) and state == 0:
		return text, 0
	...
	# 用户退出
	pattern = re.compile('.*(退出|再见|停止|休眠).*')
	result = re.search(pattern, text)
	if result:
		return '', 4
